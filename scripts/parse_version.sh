#!/usr/bin/env bash
_VERSION=$(cat "${REPO_NAME}/Telegram/SourceFiles/kotato/kotato_version.h" | grep "constexpr auto AppKotatoVersion " | awk '{print $5}' | sed 's/.$//')
_VERSIONSTR=$(cat "${REPO_NAME}/Telegram/SourceFiles/kotato/kotato_version.h" | grep "constexpr auto AppKotatoVersionStr " | awk '{print $5}' | sed 's/.$//' | cut -d'"' -f 2)
_ISBETA=$(cat "${REPO_NAME}/Telegram/SourceFiles/kotato/kotato_version.h" | grep "constexpr auto AppKotatoBetaVersion " | awk '{print $5}' | sed 's/.$//')
_BETA_SWITCH=""
